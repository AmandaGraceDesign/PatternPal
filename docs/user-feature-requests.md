...
**Status (2026-07-11): IMPLEMENTED on branch `feat/branded-banner-overlay`, AWAITING UAT + merge.** Built via spec → plan → subagent-driven execution ([spec](docs/superpowers/specs/2026-07-11-branded-banner-overlay-design.md), [plan](docs/superpowers/plans/2026-07-11-branded-banner-overlay.md)). Banner mode added to the existing watermark system: `WatermarkConfig` gains `mode`/`anchorH`/`anchorV`/`bannerTitle`/`bannerSubtitle`/`bandColor`/`bandOpacity`; `drawWatermark` gets a banner branch; shared 9-point placement also upgrades Simple-logo mode; `WatermarkPanel` mode toggle + placement grid + band controls; preview overlay parity. Social + mockup surfaces only (Cricut/EasyScale confirmed already clean, untouched). 125 tests pass, build clean. NOT yet verified on desktop/iPad and NOT merged/deployed — move to Shipped only after Mandy's UAT + merge to main + prod deploy.
